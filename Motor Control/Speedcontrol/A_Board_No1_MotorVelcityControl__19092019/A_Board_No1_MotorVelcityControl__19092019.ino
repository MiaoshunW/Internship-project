...
#define M4 A3
double kp = 0.1, ki= 0.2, kd =0.00000; // modify for optimal performance
...
void loop() { 
  currentMillis = millis();
    ...
    setpoint= 20;//*sig;//30degree
    Serial.println(dbuf);
    buf2 = buf1;
    input = dbuf ;
    buf1 = encoderPos;
    myPID.Compute(); 
    pwmOut(output);// port 1,2
    dbuf=(buf1-buf2);
    
    
    
    setpoint2= smotor2;//    X from 0.1mm to motor encoder
    input2 = encoderPos2 ;
    myPID2.Compute(); 
    pwmOut2(-output2);
    cycle++;
...
{    if (newData == true)
    {
     ...
     if (rlst==1)
     {    dataSend[0]=encoderPos;             //send motor position back
